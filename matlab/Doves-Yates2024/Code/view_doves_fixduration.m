%Example to read and display raw eye movements, fixations, and fixation durations
%an from the .mat files in Fixations folder in DOVES
...
for i = 1:length(img_filenames_list)
    ...
    for iSubj = 1:length(subj_names_list) % 
        ...
        fixDurNorm = floor(fixDur/10); %Normalize the fixation duration to a nominal MarkerSize
        for iFix = 1:length(fixDur)
           plot(fixX(iFix),fixY(iFix),'ko','MarkerFaceColor','g','MarkerSize',fixDurNorm(iFix)); axis ij;
        end
